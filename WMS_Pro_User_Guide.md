...
10. [Cycle Count / Full Count / Warehouse Map](#10-cycle-count---นับสต็อกตรวจสอบ-blind-count)
...
- **Full Count 100%** - นับเต็มรอบทั้งคลัง 349 bins พร้อม stock freeze + ส่งปรับยอด Odoo อัตโนมัติ
- **Warehouse Map** - แผนผังคลัง bird's eye view แสดงทุก bin ตาม layout จริง (9 zones, 36 racks)
...
|------|----------|--------|
| A | K2-A01-01, A-01-02, A-02-01, A-02-02 | SKINOXY Toner Pad, Hydrogel Mask |
...
5. **เดินไปที่ Pickface** — ดูตำแหน่ง Location บนหน้าจอ (เช่น K2-A01-01)
...
- ตำแหน่งจัดเก็บ (Location) เช่น K2-A01-01
...
6. **นำไปวางที่ Pickface Shelf** — เติมที่ Location ตรงตามระบบ (เช่น K2-A01-01)
...
2. เดินไปที่ Pickface Location ของ SKU (เช่น K2-A01-01)
3. **สแกน QR Code หรือ Barcode ของ Location**
   - QR format: `LOC:K2-A01-01`
...
|--------|----------|----------|
| Location QR | `LOC:K2-A01-01` | สแกน Location ที่ชั้นวาง |
...
### 10.13 Full Count 100% (นับเต็มรอบ)

**ใหม่ใน v5.1** — นับสต็อกทั้งคลัง 100% พร้อมส่งปรับยอดใน Odoo

Full Count เป็นการนับสต็อกทั้งหมดในคลัง (349 locations) ไม่ใช่แค่ sample ตาม ABC เหมาะสำหรับ:
- นับสต็อกประจำเดือน/ไตรมาส
- ตรวจสอบก่อน audit
- หลังเกิดเหตุผิดปกติ (สินค้าหาย, ย้ายคลัง)

**Tabs ในหน้า Cycle Count:**

| Tab | หน้าที่ |
|-----|---------|
| Cycle Count | นับ sample รายวัน (Blind Count เดิม) |
| Full Count | นับเต็ม 100% ทุก bin |
| Warehouse Map | แผนผังคลัง bird's eye view |

**ขั้นตอน Full Count:**

```
1. Create Session — ตั้งชื่อรอบนับ, เลือกตั้งค่า
2. Freeze Stock (ตัวเลือก) — ระงับ Pick/Pack ระหว่างนับ
3. Count — เดินนับทีละ bin, กรอกจำนวนที่นับได้
4. Reconcile — ตรวจสอบผลต่าง (variance)
5. Approve & Send to Odoo — ส่งปรับยอดเข้า Odoo
```
...
| ตัวเลือก | คำอธิบาย |
|----------|----------|
| Blind Count | ซ่อนจำนวนระบบ (ไม่เห็น System Qty ขณะนับ) |
| Double Count | ต้องนับซ้ำ 2 รอบ |
| Freeze Stock | ล็อคสต็อค — ระงับ Pick/Pack แสดง banner แดงทั้งระบบ |
...
ผลลัพธ์:
- variance = 0       → matched (ตรง)
- variancePct <= 5%   → variance-approved (ต่างน้อย อนุมัติอัตโนมัติ)
- variancePct > 5%    → needs-recount (ต้องนับซ้ำ)
```

**Odoo Integration (ส่งปรับยอด):**

เมื่อกด "Approve & Send to Odoo" ระบบจะ:
1. หา `product_id` จาก SKU (`product.product.search_read`)
2. หา `location_id` จาก location name (`stock.location.search_read`)
3. หา `stock.quant` ที่มีอยู่
4. เขียน `inventory_quantity = countedQty` (`stock.quant.write`)
5. เรียก `stock.quant.action_apply_inventory` → Odoo สร้าง stock.move ปรับยอดอัตโนมัติ

**ผลลัพธ์แสดง:**
- จำนวน adjustments ที่ส่งสำเร็จ
- จำนวนที่ล้มเหลว + สาเหตุ
- สถานะ: success / partial / error

### 10.14 Warehouse Map (แผนผังคลัง)

**ใหม่ใน v5.1** — แผนผังคลัง interactive แบบ bird's eye view

แสดงทุก 349 bins ของคลัง KOB-WH2 (Online) ตาม layout จริง:

**รูปแบบ Location:** `K2-[Zone][Rack]-[Position]` เช่น K2-A01-01

**โครงสร้างคลัง:**

| Zone | Racks | Positions/Rack | รวม |
|------|-------|----------------|-----|
| A-C | 4 racks | 8 positions | 96 bins |
| D-I | 4 racks | 10 positions | 240 bins |
| FLG | Floor Ground | 10 + FLFG | 11 bins |
| PF | Pick Face | 1 | 1 bin |
| **รวม** | | | **348+ bins** |
...
| โหมด | แสดงอะไร | สี |
|------|----------|-----|
| Stock Level | ระดับสต็อก (% ของความจุ) | เทา→แดง→ส้ม→เหลือง→เขียว |
| Count Status | สถานะการนับ Full Count | เขียว=ตรง, เหลือง=ต่าง, แดง=นับซ้ำ |
| Pick Frequency | ความถี่การหยิบ | น้ำเงิน→ม่วง→ชมพู |

**การใช้งาน:**
- **คลิก bin** → แสดง detail panel ด้านขวา (SKU, stock, lots, barcode)
- **Compact toggle** → ย่อขนาด cells สำหรับดูภาพรวม
- **Upload Floor Plan** → อัพโหลดแบบแปลนเป็น background
- **Rack rows เรียงแนวนอน** ตรงกับคลังจริง (A อยู่บนสุด, Pick Face อยู่ล่าง)
...
3. **เดินหยิบแบบ Batch** — เดินรอบเดียว หยิบสินค้าทุก Order ตาม Pick Path:
   - ที่ Location K2-A01-01 → หยิบ Toner Pad 5 ชิ้น (Order A=2, B=1, C=2) → แจกใส่ Tote ที่ถูก
...
|-----|-----------|---------|----------|
| STDH080-REFILL | Refill Toner Pad (Dewy) 150ml | 8859139111901 | K2-A01-01 |
