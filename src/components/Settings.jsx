    ...
    return (
        <section>
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-[11px] font-bold uppercase tracking-wider" style={{ color: 'var(--odoo-text-secondary)' }}>
                    Platform & Courier APIs
                </h3>
                <button onClick={handleTestAll} disabled={testingKey === 'all'} className="odoo-btn text-[10px] flex items-center gap-1 px-2 py-1" style={{ color: 'var(--odoo-purple)', borderColor: 'var(--odoo-purple)' }}>
            ...
            {/* Marketplace APIs */}
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1.5 mt-2" style={{ color: 'var(--odoo-text-muted)' }}>Marketplaces</p>
            ...
            {/* Courier APIs */}
            <p className="text-[10px] font-bold uppercase tracking-widest mb-1.5" style={{ color: 'var(--odoo-text-muted)' }}>Couriers / Logistics</p>
    ...
    return (
        <div style={{ backgroundColor: 'var(--odoo-surface)', border: '1px solid var(--odoo-border-ghost)', borderRadius: '4px', overflow: 'hidden' }}>
            <div className="px-3 py-2.5 flex items-center justify-between cursor-pointer" onClick={onToggle}
                style={{ backgroundColor: isExpanded ? 'var(--odoo-surface-low)' : 'var(--odoo-surface)', borderBottom: isExpanded ? '1px solid var(--odoo-border-ghost)' : 'none' }}>
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <div className="flex items-center gap-1.5">
                            <h4 className="text-xs font-semibold" style={{ color: 'var(--odoo-text)' }}>{def.name}</h4>
                            {isConfigured && <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'var(--odoo-success)' }} />}
                        </div>
                        <p className="text-[10px]" style={{ color: 'var(--odoo-text-secondary)' }}>
                ...
                <div className="flex items-center gap-1.5">
                    {testResult && (
                        <span className="w-2 h-2 rounded-full" style={{ backgroundColor: testResult.success ? 'var(--odoo-success)' : 'var(--odoo-danger)' }} />
                    )}
                    <svg className={`w-3.5 h-3.5 transition-transform ${isExpanded ? 'rotate-180' : ''}`} style={{ color: 'var(--odoo-text-secondary)' }}
            ...
            {isExpanded && (
                <div className="p-3 space-y-2.5 animate-slide-up">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {fields.map(field => (
                            <div key={field.key}>
                                <label className="text-[10px] font-medium mb-0.5 block" style={{ color: 'var(--odoo-text-secondary)' }}>{field.label}</label>
                                {field.type === 'select' ? (
                    ...
                    <div className="flex items-center gap-2 pt-1" style={{ borderTop: '1px solid #f0f0f0' }}>
                        ...
                        {def.docsUrl && (
                            <a href={def.docsUrl} target="_blank" rel="noopener noreferrer"
                                className="odoo-btn flex items-center gap-1 text-[11px] px-2.5 py-1.5" style={{ color: 'var(--odoo-text-secondary)' }}>
                                <ExternalLink className="w-3 h-3" /> Docs
                            </a>
                        )}
                        {testResult && (
                            <span className="text-[11px] font-medium ml-1" style={{ color: testResult.success ? 'var(--odoo-success)' : 'var(--odoo-danger)' }}>
...
const SecurityAuditPanel = ({ triggerConfirm }) => {
    ...
    return (
        <section>
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-[11px] font-bold uppercase tracking-wider flex items-center gap-1.5" style={{ color: 'var(--odoo-text-secondary)' }}>
                    <Shield className="w-3.5 h-3.5" /> Security Audit Log
                </h3>
                <div className="flex items-center gap-2">
                    <button onClick={() => setExpanded(!expanded)} className="odoo-btn text-[10px] px-2 py-1" style={{ color: 'var(--odoo-purple)', borderColor: 'var(--odoo-purple)' }}>
                        {expanded ? 'Collapse' : `Show (${logs.length})`}
                    </button>
                    {logs.length > 0 && (
                        <button onClick={handleClear} className="odoo-btn text-[10px] px-2 py-1 flex items-center gap-1" style={{ color: 'var(--odoo-danger)', borderColor: '#f5c6cb', backgroundColor: '#fff5f5' }}>
            ...
            {expanded && (
                <div className="rounded overflow-hidden" style={{ border: '1px solid var(--odoo-border-ghost)' }}>
                    {logs.length === 0 ? (
                        <div className="p-4 text-center text-xs" style={{ color: 'var(--odoo-text-secondary)' }}>No audit log entries</div>
                    ) : (
                            ...
                            <table className="w-full text-xs" style={{ borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ backgroundColor: 'var(--odoo-surface-low)', borderBottom: '1px solid var(--odoo-border-ghost)' }}>
                                        <th className="text-left px-3 py-2 font-semibold" style={{ color: 'var(--odoo-text)' }}>Timestamp</th>
                                        <th className="text-left px-3 py-2 font-semibold" style={{ color: 'var(--odoo-text)' }}>Action</th>
                                        <th className="text-left px-3 py-2 font-semibold" style={{ color: 'var(--odoo-text)' }}>User</th>
                                        <th className="text-left px-3 py-2 font-semibold" style={{ color: 'var(--odoo-text)' }}>Details</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {logs.map((entry, i) => {
            ...
            <div style={{ backgroundColor: 'var(--odoo-surface)', border: '1px solid var(--odoo-border-ghost)', borderRadius: '4px', overflow: 'hidden' }}>
                <div className="px-5 py-3.5" style={{ borderBottom: '1px solid var(--odoo-border-ghost)', backgroundColor: 'var(--odoo-surface-low)' }}>
                    <h2 className="text-sm font-semibold flex items-center gap-2" style={{ color: 'var(--odoo-text)' }}>
                        <SettingsIcon className="w-4 h-4" style={{ color: 'var(--odoo-text-secondary)' }} /> {t('sysSettings')}
                    </h2>
                </div>
                <div className="p-5 space-y-6">
                    {/* Preferences */}
                    <section>
                        <h3 className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: 'var(--odoo-text-secondary)' }}>{t('pref')}</h3>
                        <div className="p-4 rounded" style={{ backgroundColor: 'var(--odoo-surface-low)', border: '1px solid var(--odoo-border-ghost)' }}>
                            <label className="text-xs font-semibold mb-1.5 flex items-center gap-1.5" style={{ color: 'var(--odoo-text)' }}>
                            ...
                            <p className="text-xs mt-1.5" style={{ color: 'var(--odoo-text-secondary)' }}>{t('langDesc')}</p>
                        </div>
                    </section>
                    ...
                    {userRole === 'admin' && (
                        <section>
                            <h3 className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: 'var(--odoo-text-secondary)' }}>{t('apiInt')}</h3>
                            <div className="space-y-2">
                                {[
                                    ...
                                    <div key={api.id} style={{ backgroundColor: 'var(--odoo-surface)', border: '1px solid var(--odoo-border-ghost)', borderRadius: '4px', overflow: 'hidden' }}>
                                        <div className="px-4 py-3 flex items-center justify-between" style={{ backgroundColor: 'var(--odoo-surface-low)', borderBottom: apiConfigs[api.id].enabled ? '1px solid var(--odoo-border-ghost)' : 'none' }}>
                                            <div className="flex items-center gap-2.5">
                                                <div className="p-1.5 rounded shrink-0" style={{ backgroundColor: '#f0e8ed', color: 'var(--odoo-purple)' }}>{api.icon}</div>
                                                <div>
                                                    <h4 className="text-sm font-semibold" style={{ color: 'var(--odoo-text)' }}>{api.name}</h4>
                                                    <p className="text-xs" style={{ color: 'var(--odoo-text-secondary)' }}>{api.desc}</p>
                                                </div>
                                            </div>
                                        ...
                                        {apiConfigs[api.id].enabled && (
                                            <div className="p-4 space-y-3 animate-slide-up">
                                                {api.id === 'odoo' && (
                                                    <div className="flex items-center gap-3 pb-3" style={{ borderBottom: '1px solid var(--odoo-border-ghost)' }}>
                                                        <button
                                                ...
                                                {api.id === 'odoo' && (
                                                    <div className="space-y-2 pt-1">
                                                        <div className="flex flex-wrap items-center gap-2">
                                                            ...
                                                            {testResult && (
                                                                <div className="flex items-center gap-1.5 text-xs font-medium" style={{ color: testResult.success ? 'var(--odoo-success)' : 'var(--odoo-danger)' }}>
                                                                    {testResult.success ? <CheckCircle2 className="w-3.5 h-3.5" /> : <AlertCircle className="w-3.5 h-3.5" />}
                                                                    {testResult.message}
                                                                </div>
                                                            )}
                                                        </div>
                                                        <div className="flex flex-wrap items-center gap-2 pt-1" style={{ borderTop: '1px solid var(--odoo-border-ghost)' }}>
                                                            <button
                                                                onClick={async () => {
                                                                ...
                                                                style={{ backgroundColor: '#e8f5e9', color: '#2e7d32', borderColor: 'var(--odoo-success)' }}
                                                            >
                                                                {isPickfaceSetup ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <MapPin className="w-3.5 h-3.5" />}
                                                                Setup PICKFACE Location
                                                            </button>
                                                            {pickfaceResult && (
                                                                <div className="flex items-center gap-1.5 text-xs font-medium" style={{ color: pickfaceResult.success ? 'var(--odoo-success)' : 'var(--odoo-danger)' }}>
                                                        ...
                                                        {/* Create Test SO */}
                                                        <div className="pt-2" style={{ borderTop: '1px solid var(--odoo-border-ghost)' }}>
                                                            <p className="text-[11px] font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--odoo-text-secondary)' }}>Create Test Sales Orders (SKINOXY)</p>
                                                            <div className="flex items-center gap-2 flex-wrap">
                                                                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded" style={{ backgroundColor: 'var(--odoo-surface-low)', border: '1px solid var(--odoo-border-ghost)' }}>
                                                                    <span className="text-xs" style={{ color: 'var(--odoo-text-secondary)' }}>SO Count:</span>
                                                                    <input
                                                                ...
                                                                {createSOResult && (
                                                                    <div className="flex items-center gap-1.5 text-xs font-medium" style={{ color: createSOResult.success ? 'var(--odoo-success)' : 'var(--odoo-danger)' }}>
                                                                        {createSOResult.success ? <CheckCircle2 className="w-3.5 h-3.5" /> : <AlertCircle className="w-3.5 h-3.5" />}
                                                                        {createSOResult.message}
                                                                    </div>
                                                                )}
                                                            </div>
                                                            <p className="text-[11px] mt-1" style={{ color: 'var(--odoo-text-muted)' }}>Products: STDH080-REFILL, STBG080, SWB700, SWH700 — random qty 1-3</p>
                                                        </div>
                                                    </div>
                                                )}
                                            </div>
                    ...
                    {/* Data Management */}
                    <section>
                        <h3 className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: 'var(--odoo-text-secondary)' }}>{t('dataMgmt')}</h3>
                        <div className="p-4 rounded space-y-4" style={{ backgroundColor: 'var(--odoo-surface-low)', border: '1px solid var(--odoo-border-ghost)' }}>
                            <div>
                                <label className="text-xs font-semibold mb-1.5 block" style={{ color: 'var(--odoo-text)' }}>{t('workDate')}</label>
                                <input type="date" value={workDate} onChange={e => setWorkDate(e.target.value)} className="odoo-input w-full md:w-1/2" />
                            </div>
                            {userRole === 'admin' && (
                                <div className="pt-4" style={{ borderTop: '1px solid var(--odoo-border-ghost)' }}>
                                    <h4 className="text-sm font-semibold mb-0.5" style={{ color: 'var(--odoo-danger)' }}>{t('dangerZone')}</h4>
                                    <p className="text-xs mb-3" style={{ color: 'var(--odoo-text-secondary)' }}>{t('clearDataDesc')}</p>
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            ...
                                            style={{ backgroundColor: '#fff5f5', color: 'var(--odoo-danger)', borderColor: '#f5c6cb' }}
                                        >
